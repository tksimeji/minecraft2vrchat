...
namespace M2V.Editor.Model
{
    internal interface IAssetReader
    {
        ...
        bool TryReadBytes(string path, out byte[] bytes);
        IEnumerable<string> EnumeratePaths(string root, string extension);
    }

    internal sealed class DirectoryAssetReader : IAssetReader
    {
        private readonly DirectoryInfo _root;

        internal DirectoryAssetReader(DirectoryInfo root)
        {
            _root = root;
        }
        ...
        public IEnumerable<string> EnumeratePaths(string root, string extension)
        {
    ...
    internal sealed class ZipAssetReader : IAssetReader, IDisposable
    {
    ...
    internal sealed class CompositeAssetReader : IAssetReader, IDisposable
    {
        private readonly List<IAssetReader> _readers;

        internal CompositeAssetReader(IEnumerable<IAssetReader> readers)
        {
            _readers = readers == null ? new List<IAssetReader>() : new List<IAssetReader>(readers);
        }
